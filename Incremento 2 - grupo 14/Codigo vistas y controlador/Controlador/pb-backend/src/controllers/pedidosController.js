...
async function checklist(req, res) {
  ...
  try {
    ...
    // CU-108 / CU-117: agrupar por bodega para facilitar recolección
    // Nota: inventario_bodega no tiene FK a anaquel, se muestra solo la bodega
    ...
    const { rows: stockOtrasBodegas } = await query(
      `SELECT ib.material_sku AS sku,
       ...
       JOIN material_orden_trabajo mot
            ON mot.material_sku = ib.material_sku
           AND mot.orden_trabajo_id_orden = $1
       WHERE (ib.inventario_bodega_cantidad_fisica - ib.inventario_bodega_cantidad_reservada) > 0`,
      [id]
    );
    const stockAlternativo = {};
    for (const s of stockOtrasBodegas) {
      if (!stockAlternativo[s.sku]) stockAlternativo[s.sku] = [];
      stockAlternativo[s.sku].push({ bodega: s.bodega, disponible: parseFloat(s.disponible) });
    }
...
async function reportarDiscrepancia(req, res) {
  ...
  try {
    ...
    const { rows: mat } = await query(
      `SELECT material_sku FROM material_orden_trabajo
    ...
    await query(
      `INSERT INTO alerta_faltante_pedido (
       ...
       ) VALUES ($1, 0, 0, 'activa', $2, $3, $4)`,
      [stockActual[0]?.disponible || 0, sku, ot[0].proyecto_id, userId]
    );
    ...
    const { rows: otrasBodegas } = await query(
      `SELECT b.bodega_nombre_bodega AS bodega,
              ib.inventario_bodega_cantidad_fisica - ib.inventario_bodega_cantidad_reservada AS cantidad,
              u.material_unidad_medida_nombre AS unidad
       FROM inventario_bodega ib
       JOIN bodega b ON b.bodega_id_bodega = ib.bodega_id_bodega
       ...
       WHERE ib.material_sku = $1
         AND (ib.inventario_bodega_cantidad_fisica - ib.inventario_bodega_cantidad_reservada) > 0`,
      [sku]
    );
...
module.exports = { listar, checklist, reservar, reportarDiscrepancia };
