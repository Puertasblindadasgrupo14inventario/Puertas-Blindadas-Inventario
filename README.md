...
│   └── ...
│
└── Incremento 2 - grupo 14/
    ├── Incremento 2 - Grupo 14.docx           ← Documento principal incremento 2
    ├── Incremento 2 Grupo 14 Documento de instalacion.docx
    ...
    │   │       ├── middleware/
    │   │       └── db/
    │   └── Vistas/                   ← Frontend web (React + Vite)
    │       ├── package.json
    │       └── src/
    │           ├── pages/
    │           │   ├── login/
    ...
    │           └── shared/
    │
    ├── Diagramas CU incremento 2/
    ├── Diagramas Secuencia/          ← Diagramas .drawio + imágenes
...
> A partir del Incremento 2 el frontend es una app **React (Vite)**, por lo que ya no se necesita Python para levantar las vistas.
...
```bash
cd "Incremento 2 - grupo 14/Codigo vistas y controlador/Controlador"
npm install
```
...
Dentro de `Incremento 2 - grupo 14/Codigo vistas y controlador/Controlador/`, crear un archivo llamado exactamente `.env` (sin nombre antes del punto) con el siguiente contenido:
...
>
> Reemplazar `DB_PASSWORD` por la contraseña definida al instalar PostgreSQL (paso 2).
...
```bash
cd "Incremento 2 - grupo 14/Codigo vistas y controlador/Vistas"
npm install
npm run dev
```
...
Ya vienen cargados por el DML:

| Username | Contraseña | Rol | Estado |
|---|---|---|---|
| `alvaro.morales` | `password123` | Gerencia | Activa |
| `silgod` | `silgod1234` | JOP | Activa |

> En caso de tener problemas con el usuario JOP, crear uno a través de la interfaz web (o manualmente, ver sección siguiente).
...
- Navegar a **Bodega** debe mostrar las bodegas actuales
...
| **Login funciona pero dashboard vacío** | Backend no levantado o puerto incorrecto | Verificar que `npm run dev` (Controlador) esté corriendo en el puerto 3000 |
| **Frontend no carga / puerto 5173 no responde** | Dependencias del frontend no instaladas o `npm run dev` no ejecutado en `Vistas/` | Ejecutar `npm install` y `npm run dev` dentro de `Vistas/` |
...
- **Frontend (Incremento 2):** Migrado de HTML/CSS/JS vanilla a **React + Vite**, servido con `npm run dev` en el puerto 5173.
...
**Scrum++** con dos semestres de desarrollo iterativo. Incremento 2: iteraciones de pruebas de caja negra sobre el sistema completo (backend + frontend React + base de datos).
